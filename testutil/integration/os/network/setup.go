...
import (
	...
	exampleapp "github.com/realiotech/realio-network/example_chain"
...
func setDefaultAssetGenesisState(evmosApp *exampleapp.ExampleChain, genesisState evmostypes.GenesisState) evmostypes.GenesisState {
	// NOTE: here we are using the setup from the example chain
	assetGen := exampleapp.NewAssetGenesisState()

	genesisState[assettypes.ModuleName] = evmosApp.AppCodec().MustMarshalJSON(assetGen)
	return genesisState
}
