...
var (
	V1UpgradeName       = "v1"
	V1ForkHeight        = int64(5989487)
	V1oneEnternityLater = time.Date(9999, 9, 9, 9, 9, 9, 9, time.UTC)
)
var v1Fork = Fork{
	UpgradeName: V1UpgradeName,
	UpgradeHeight: V1ForkHeight,
	BeginForkLogic: V1RunForkLogic,
}
...
func V1RunForkLogic(ctx sdk.Context, app *RealioNetwork) {
	if ctx.BlockHeight() == V1ForkHeight {
		fmt.Println("upgrading v1")
...
// func removeDuplicateValueRedelegationQueueKey(app *baseapp.RealioNetwork, ctx sdk.Context) {
...
// func removeDuplicateUnbondingValidator(app *baseapp.RealioNetwork, ctx sdk.Context) {
...
// func removeDuplicateValueUnbondingQueueKey(app *baseapp.RealioNetwork, ctx sdk.Context) {
// 	// Get Staking keeper, codec and staking store
// 	sk := app.StakingKeeper
// 	cdc := app.AppCodec()
// 	store := ctx.KVStore(app.keys[stakingtypes.ModuleName])
